...
{
	char corrected[PENCIL_MAX_PATH+1];
	strcpy(corrected, relative);
	CorrectSlashes(corrected);
...
{
	char strJPGPath[PENCIL_MAX_PATH] = {0};
	char strPNGPath[PENCIL_MAX_PATH] = {0};
	char strTGAPath[PENCIL_MAX_PATH]    = {0};
	char strBMPPath[PENCIL_MAX_PATH]    = {0};
	FILE *fp = NULL;

	//GetCurrentDirectory(PENCIL_MAX_PATH, strJPGPath);
...
int SavePNG2(const char* fullpath, LoadedTex* image)
{
	...
	char srcstr[123];
	sprintf(srcstr, "Rendered using %s Version %d", TITLE, VERSION);
#ifdef USESTEAM
