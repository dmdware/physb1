...
Camera::Camera(FixFrac posx, FixFrac posy, FixFrac posz, FixFrac viewx, FixFrac viewy, FixFrac viewz, FixFrac upx, FixFrac upy, FixFrac upz)
{
	position(posx, posy, posz, viewx, viewy, viewz, upx, upy, upz);
}

void Camera::position(FixFrac posx, FixFrac posy, FixFrac posz, FixFrac viewx, FixFrac viewy, FixFrac viewz, FixFrac upx, FixFrac upy, FixFrac upz)
{
...
void Camera::rotatebymouse(int dx, int dy)
{
	...
	FixFrac angleY = (FixFrac)( -dx ) * MOUSE_SENSITIVITY;
	FixFrac angleZ = (FixFrac)( -dy ) * MOUSE_SENSITIVITY;

	static FixFrac lastRotX = 0.0f; 
	...
	if(m_orientv.x > 1.0f)     
	{
		...
		if(lastRotX != 1.0f) 
		{
			...
			// rotate the camera by the remaining angle (FixFrac(1.0f) - lastRotX)
			rotateview( FixFrac(1.0f) - lastRotX, vAxis.x, vAxis.y, vAxis.z);
		}
	}
	...
	else if(m_orientv.x < -1.0f)
	{
		...
		if(lastRotX != -1.0f)
		{
			...
			// rotate the camera by ( FixFrac(-1.0f) - lastRotX)
			rotateview( FixFrac(-1.0f) - lastRotX, vAxis.x, vAxis.y, vAxis.z);
		}
	}
...
void Camera::rotateabout(Vec3f center, FixFrac rad, FixFrac x, FixFrac y, FixFrac z)
{
	m_view = RotateAround(m_view, center, rad, x, y, z);
	m_pos = RotateAround(m_pos, center, rad, x, y, z);
	//m_up = RotateAround(m_up + m_pos, center, rad, x, y, z) - m_pos;
...
void Camera::limithvel(FixFrac vLimit)
{
	Vec3f hVel = Vec3f(m_vel.x, 0, m_vel.z);
	FixFrac vVel = MAG_VEC3F( hVel );
...
void Camera::rotateview(FixFrac angle, FixFrac x, FixFrac y, FixFrac z)
{
	Vec3f vNewView;

	Vec3f vView = m_view - m_pos;		

	FixFrac cosTheta = (FixFrac)cos(angle);
	FixFrac sinTheta = (FixFrac)sin(angle);

	vNewView.x  = (cosTheta + (FixFrac(1) - cosTheta) * x * x)		* vView.x;
	vNewView.x += ((FixFrac(1) - cosTheta) * x * y - z * sinTheta)	* vView.y;
	vNewView.x += ((FixFrac(1) - cosTheta) * x * z + y * sinTheta)	* vView.z;

	vNewView.y  = ((FixFrac(1) - cosTheta) * x * y + z * sinTheta)	* vView.x;
	vNewView.y += (cosTheta + (FixFrac(1) - cosTheta) * y * y)		* vView.y;
	vNewView.y += ((FixFrac(1) - cosTheta) * y * z - x * sinTheta)	* vView.z;

	vNewView.z  = ((FixFrac(1) - cosTheta) * x * z - y * sinTheta)	* vView.x;
	vNewView.z += ((FixFrac(1) - cosTheta) * y * z + x * sinTheta)	* vView.y;
	vNewView.z += (cosTheta + (FixFrac(1) - cosTheta) * z * z)		* vView.z;
...
/*
void Camera::Strafe(FixFrac speed)
{
	m_vel.x += m_strafe.x * speed;
	m_vel.z += m_strafe.z * speed;
}

void Camera::Move(FixFrac speed)
{
...
void Camera::move(FixFrac speed)
{
	...
	m_pos.y += vVector.y * speed;
	m_view.x += vVector.x * speed;
	m_view.z += vVector.z * speed;
	m_view.y += vVector.y * speed;
}

//bool g_debug2 = false;

void Camera::accelerate(FixFrac speed)
{
	Vec3f vVector = m_view - m_pos;
...
void Camera::accelstrafe(FixFrac speed)
{
...
void Camera::accelrise(FixFrac speed)
{
...
/*
void Camera::Rise(FixFrac speed)
{
	Vec3f vVector = m_up;
...
void Camera::calcstrafe() 
{
	...
	m_strafe = Normalize(vCross);
 	//vCross = Cross(m_strafe, m_view - m_pos);
	//m_up = Normalize(vCross);
}
