...
RichPart::RichPart(int32_t type, int32_t subtype)
{
	if(type == RICH_ICON)
	{
		m_type = RICH_ICON;
...
int32_t RichPart::texlen() const	// icons count as 1 glyph
{
	if(m_type == RICH_TEXT)
...
int32_t RichPart::rawlen() const	// icon tag length is counted
{
	if(m_type == RICH_TEXT)
		return m_text.m_length;
	else if(m_type == RICH_ICON)
...
RichPart RichPart::substr(int32_t start, int32_t length) const
{
	if(m_type == RICH_ICON)
		return *this;
...
RichText RichText::operator+(const RichText &other)
{
	...
	std::list<RichPart>::reverse_iterator i;
	std::list<RichPart>::const_iterator i2;
	...
	if(m_part.size() > 0 && other.m_part.size() > 0)
	{
		...
		for(i=m_part.rbegin(); i!=m_part.rend(); i++)
		{
			if(i->texlen() > 0)
			{
				last1 = &*i;
		...
		for(i2=other.m_part.begin(); i2!=other.m_part.end(); i2++)
		{
			if(i2->texlen() > 0)
			{
				first2 = (RichPart*)&*i2;
	...
	for(i2=m_part.begin(); i2!=m_part.end(); i2++)
	{
...
RichText RichText::substr(int32_t start, int32_t length) const
{
	RichText retval;

	int32_t totallen = texlen();
	int32_t currplace = 0;
	std::list<RichPart>::const_iterator currp = m_part.begin();

	while(currplace < totallen && currplace < start+length && currp != m_part.end())
	{
		int32_t currlen = currp->texlen();

		if(currlen <= 0)
			continue;

		int32_t startplace = start - currplace;
		int32_t endplace = (start+length) - currplace;
		...
		if(startplace < currlen && endplace >= 0)
		{
			...
			int32_t addlen = endplace - startplace;
...
std::string RichText::rawstr() const
{
	...
	std::list<RichPart>::const_iterator i = m_part.begin();
...
int32_t RichText::texlen() const	// icons count as 1 glyph
{
	int32_t runl = 0;
...
int32_t RichText::rawlen() const	// icon tag length is counted
{
	int32_t runl = 0;

	std::list<RichPart>::const_iterator i=m_part.begin();

	for(i=m_part.begin(); i!=m_part.end(); i++)
		runl += i->rawlen();
...
RichText RichText::pwver() const	//asterisk-mask password std::string
{
	...
	std::list<RichPart>::const_iterator wi=m_part.begin();

	for(; wi!=m_part.end(); wi++)
		len += wi->texlen();
...
RichText ParseTags(RichText original, int32_t* caret)
{
	...
	std::list<RichPart>::iterator i;

	for(i=original.m_part.begin(); i!=original.m_part.end(); i++)
	{
		if(i->m_type == RICH_TEXT)
		{
			...
			for(int32_t j=0; j<s->m_length; j++)
			{
				if(u[j] == '&' &&
					!foundtag)
				{
			...
			uint32_t addi = StrToInt(num.c_str());
			RichPart addp(addi);
			parsed = parsed + RichText(addp);

			int32_t taglen = lastof - firstof + 1;
			int32_t partlen =  i->m_text.m_length;
	...
	for(i=original.m_part.begin(); i!=original.m_part.end(); i++)
	{
		if(i->m_type == RICH_TEXT)
		{
			...
			for(int32_t j=0; j<ICONS; j++)
			{
				Icon* icon = &g_icon[j];
				int32_t firstof = i->m_text.firstof(icon->m_tag);
				...
				int32_t taglen = icon->m_tag.m_length;
				int32_t partlen =  i->m_text.m_length;
